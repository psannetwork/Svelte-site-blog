...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	if (!filenameRaw) throw error(400, 'Filename required');

	// Path traversal 対策: .. を除去し、拡張子 .db または .db.bak のみ許可
	const filename = filenameRaw.replace(/\.\.+/g, '').replace(/[^\w\s\-\.]/g, '');

	if (!filename.endsWith('.db') && !filename.endsWith('.db.bak')) {
		throw error(400, 'Invalid database file');
	}
			...
			try {
				...
				try {
					...
					tableNames = pragmaTables
						.filter(
							(t) => t.type === 'table' && t.schema === 'main' && !t.name.startsWith('sqlite_')
						)
						.map((t) => t.name);
				} catch (e) {
				...
				if (tableNames.length === 0) {
					const masterTables = tempDb
						.prepare(
							"SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'"
						)
						.all() as any[];
					tableNames = masterTables.map((t) =>
						typeof t === 'string' ? t : t.name || t.NAME || (Object.values(t)[0] as string)
					);
				}
