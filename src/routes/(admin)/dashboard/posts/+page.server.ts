...
export const actions: Actions = {
	updateStatus: async ({ request, locals }) => {
		if (!locals.user || (locals.user.role !== 'admin' && locals.user.role !== 'editor')) return fail(403);
		
		const formData = await request.formData();
		const id = formData.get("id") as string;
		const status = formData.get("status") as string;

		db.prepare("UPDATE post SET visibility = ?, updated_at = ? WHERE id = ?").run(status, Date.now(), id);
		return { success: true };
	},
