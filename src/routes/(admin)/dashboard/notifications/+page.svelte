...
<div class="max-w-4xl mx-auto space-y-8">
	...
	<div class="card-psan divide-y border-[--border-color]">
		{#await fetch('/api/notifications').then(r => r.json())}
			<div class="p-10 text-center text-muted animate-pulse">Loading notifications...</div>
		{:then result}
			{#if result.success}
				{#if result.notifications && result.notifications.length > 0}
					{#each result.notifications as note}
						<a href={note.link || '#'} class="block p-6 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors {note.is_read ? 'opacity-60' : 'bg-green-50/50 dark:bg-green-900/10'}">
							<div class="flex items-start gap-4">
								<div class="p-2 rounded-xl {note.type === 'reply' ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-600'}">
									{#if note.type === 'reply'}
								...
								<div class="flex-1 min-w-0">
									...
									<p class="text-xs font-black text-muted uppercase tracking-wider">{new Date(note.created_at).toLocaleString()}</p>
								</div>
								{#if !note.is_read}
					...
					{/each}
				{:else}
					<div class="p-20 text-center text-muted font-bold opacity-50">
