...
<div class="min-h-screen bg-white dark:bg-slate-950 flex transition-colors duration-500">
	<!-- Sidebar (Desktop) -->
	<aside
		class="hidden lg:flex w-80 bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 flex-col sticky top-0 h-screen z-50 transition-all duration-500 shadow-2xl shadow-slate-200/50 dark:shadow-none"
	>
		<div class="p-10">
			<div class="flex items-center gap-4 mb-10">
				<div
					class="w-12 h-12 bg-psan-green rounded-2xl rotate-12 flex items-center justify-center text-white shadow-lg shadow-psan-green/20"
				>
					<svg class="w-7 h-7 -rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"
						><path
				...
				<div>
					...
					<p class="text-[8px] font-black text-psan-green uppercase tracking-widest">
						{t(lang, 'admin_console')}
					</p>
				</div>
			</div>
			...
			<a
				...
				class="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-400 hover:text-psan-green transition-colors group"
			>
				<svg
		...
		<div class="p-6 mt-auto">
			<div class="bg-slate-100 dark:bg-slate-800/50 rounded-3xl p-4 flex items-center gap-4">
				<div
					class="w-10 h-10 rounded-xl bg-psan-green/20 flex items-center justify-center text-psan-green font-black"
				>
				...
				<div class="min-w-0">
					<div class="text-[10px] font-black text-slate-800 dark:text-slate-200 truncate uppercase">
						{data.user?.nickname || data.user?.username}
					</div>
					<div class="text-[8px] font-black text-psan-green uppercase tracking-widest">
						{data.user?.role}
					</div>
				</div>
			</div>
	...
	<!-- Mobile Nav -->
	<div
		class="lg:hidden fixed bottom-6 left-6 right-6 h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200 dark:border-slate-800 rounded-[32px] flex items-center justify-around px-4 z-50 shadow-2xl"
	>
		<a href="/" data-sveltekit-reload class="p-3 text-slate-400" aria-label={t(lang, 'visit_site')}>
		...
		{#each mobileItems as item}
			...
			<a
				href={item.href}
				class="p-4 transition-all relative {isActive
					? 'text-psan-green scale-110'
					: 'text-slate-400'}"
			...
			>
				...
				{#if isActive}
					<div
						class="absolute -top-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-psan-green rounded-full"
					></div>
				{/if}
			</a>
		{/each}
		<a
			href="/dashboard/settings"
			class="p-3 text-slate-400 {page.url.pathname === '/dashboard/settings'
				? 'text-psan-green'
				: ''}"
