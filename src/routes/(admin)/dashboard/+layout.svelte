...
<div class="flex h-screen bg-slate-50 dark:bg-[#0b0f1a] transition-colors overflow-hidden font-sans">
					...
					<a
						...
						{isDisabled
							...
							: isActive
								...
								: 'text-muted dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800/50 hover:text-main dark:hover:text-white'}"
					>
	...
	<div class="flex-1 flex flex-col min-w-0 overflow-hidden relative">
		...
		<header class="h-24 px-6 md:px-12 flex items-center justify-between shrink-0 z-40">
			<div class="flex items-center gap-6">
				<div class="lg:hidden flex items-center gap-3">
					...
					<span class="font-black text-lg tracking-tighter text-main uppercase">Dashboard</span>
				</div>
				<div class="hidden lg:flex items-center gap-2 px-6 py-2 bg-white dark:bg-slate-800 rounded-full border border-slate-100 dark:border-slate-700 shadow-sm">
					...
					<span class="text-[10px] font-black text-muted uppercase tracking-widest">System Online</span>
				</div>
			</div>
			...
			<div class="flex items-center gap-4 md:gap-6">
				<button
					...
					class="w-12 h-12 rounded-2xl flex items-center justify-center transition-all bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 text-main hover:border-psan-green shadow-sm"
				>
		...
		<!-- CONTENT -->
		<main class="flex-1 overflow-y-auto px-6 md:px-12 pb-32 pt-4 custom-scrollbar">
		...
		<!-- MOBILE BOTTOM NAV -->
		<nav class="lg:hidden fixed bottom-6 left-6 right-6 h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-white/20 dark:border-slate-800/50 rounded-[32px] shadow-2xl z-50 flex items-center justify-around px-4">
			{#each mobileItems as item}
				{@const isActive = page.url.pathname === item.href}
				<a
					href={item.href}
					class="flex flex-col items-center justify-center gap-1 transition-all
					{isActive ? 'text-psan-green scale-110' : 'text-slate-400 dark:text-slate-600'}"
				>
			...
			<a
				href="/account"
				class="flex flex-col items-center justify-center gap-1 text-slate-400 dark:text-slate-600"
			>
