<script lang="ts">
	...
	// data: URLなどの巨大な文字列を短縮して表示を軽くする
	function truncateDataUrls(content: string): string {
		if (!content) return '';
		// data: で始まり、100文字以上続く base64 などを短縮
		return content.replace(/data:[^;]+;base64,[a-zA-Z0-9+/=]{100,}/g, (match) => {
			return match.substring(0, 50) + '...[DATA TRUNCATED]...';
		});
	}
</script>
	...
	<div class="grid gap-6">
		{#each data.comments as comment}
			...
			<div
				...
				{comment.parent_id
					? 'border-l-psan-pink/40 bg-psan-pink/[0.01]'
					: 'border-l-psan-green/40 bg-psan-green/[0.01]'} hover:border-l-psan-pink active:scale-[0.99]"
			>
				<div class="flex items-start md:items-center gap-6 flex-1 min-w-0">
					<!-- AVATAR with Status Icon -->
					<div class="relative shrink-0">
						<div
							class="w-16 h-16 md:w-20 md:h-20 rounded-[24px] bg-white dark:bg-slate-800 overflow-hidden border border-slate-100 dark:border-slate-700 flex items-center justify-center group-hover:scale-105 transition-transform shadow-sm"
						>
						...
						{#if comment.parent_id}
							<div
								class="absolute -right-2 -bottom-1 w-7 h-7 bg-psan-pink text-white rounded-full flex items-center justify-center shadow-lg ring-4 ring-white dark:ring-slate-900 scale-90 md:scale-100"
							>
					...
					<div class="flex-1 min-w-0">
						<div class="flex flex-wrap items-center gap-x-4 gap-y-1 mb-3">
							<span class="text-base font-black text-main uppercase tracking-tight"
								>{comment.author_name}</span
							>

							{#if comment.parent_author_name}
								<span
									class="text-[10px] font-black text-psan-pink uppercase tracking-widest flex items-center gap-1.5"
								>
						...
						{#if comment.parent_content}
							<div class="mb-4 relative">
								<div
									class="p-3 bg-slate-100/50 dark:bg-slate-800/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 text-xs text-muted font-medium line-clamp-1 italic pl-4 border-l-4 border-l-psan-pink/20"
								>
						...
						{/if}
												...
												<!-- MAIN COMMENT TEXT -->
												<div class="relative">
													<p class="text-base md:text-lg font-bold text-main leading-relaxed group-hover:text-psan-pink transition-colors break-words">
														<!-- eslint-disable-next-line svelte/no-at-html-tags -->
														{@html sanitizeHtml(truncateDataUrls(comment.content))}
													</p>
												</div>
						...
						<div class="mt-4 flex items-center gap-2">
							<div
								class="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg flex items-center gap-2 max-w-full"
							>
								<svg
									class="w-3 h-3 text-psan-green shrink-0"
									fill="none"
									stroke="currentColor"
									viewBox="0 0 24 24"
									><path
									...
									/></svg
								>
								<span class="text-[9px] font-black text-muted uppercase tracking-widest truncate"
				...
				<!-- ACTIONS -->
				<div
					class="flex items-center justify-end gap-3 border-t md:border-t-0 pt-6 md:pt-0 border-slate-100 dark:border-slate-800"
				>
					<form
						method="POST"
