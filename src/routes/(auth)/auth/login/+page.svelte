...
<div class="w-full max-w-md px-4 py-20">
	<div class="card-psan p-8 md:p-12 shadow-2xl shadow-psan-green/5 relative overflow-hidden group">
		<!-- 装飾的な背景要素 -->
		<div
			class="absolute -top-20 -right-20 w-40 h-40 bg-psan-green/10 rounded-full blur-3xl group-hover:bg-psan-green/20 transition-colors duration-700"
		></div>
		...
		<div class="relative z-10 text-center mb-10">
			...
			<h2 class="text-3xl font-black tracking-tighter uppercase text-main">Welcome Back</h2>
			<p class="mt-2 text-sm font-medium text-muted">ログインしてダッシュボードにアクセス</p>
		</div>

		<form method="POST" use:enhance class="space-y-6 relative z-10">
			<div class="space-y-4">
				<div class="space-y-2">
					<label for="username" class="text-[10px] font-black tracking-widest text-muted uppercase"
						>User ID</label
					>
					<input
						id="username"
						...
						type="text"
						required
						class="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-none rounded-2xl p-4 font-bold focus:ring-2 ring-psan-green transition-all text-main"
						placeholder="Username"
					/>
				</div>
				<div class="space-y-2">
					<label for="password" class="text-[10px] font-black tracking-widest text-muted uppercase"
						>Password</label
					>
					<input
						id="password"
			...
			<button type="submit" class="btn-psan-primary w-full py-4 text-lg"> Login </button>
		</form>
		...
		>
			...
			<a
				href="/"
				class="block text-[10px] font-black tracking-widest text-muted hover:text-psan-green transition-colors uppercase"
				>Back to Home</a
			>
		</div>
	</div>
