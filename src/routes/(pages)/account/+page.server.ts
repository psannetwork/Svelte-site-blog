...
export const actions: Actions = {
	updateNickname: async ({ request, locals }) => {
		...
		db.prepare(
			'UPDATE user SET nickname = ?, avatar_url = ?, notification_enabled = ? WHERE id = ?'
	...
	updatePassword: async ({ request, locals }) => {
		...
		if (newPassword.length < 6)
			return fail(400, { message: '新しいパスワードは6文字以上である必要があります。' });
	...
	deleteAccount: async ({ locals, cookies }) => {
		...
		const user = db
			.prepare('SELECT is_protected FROM user WHERE id = ?')
			.get(locals.user.id) as any;
		if (user?.is_protected)
			return fail(400, {
				message: 'このアカウントはシステムによって保護されており、削除できません。'
			});
