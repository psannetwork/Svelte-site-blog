...
<header class="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl sticky top-0 z-50 border-b border-slate-200 dark:border-slate-800 transition-colors">
	<nav class="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between">
		...
		<div class="flex items-center gap-4">
			<button onclick={() => theme.toggle()} class="p-3 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:scale-110 transition-all" aria-label="テーマ切替">
	...
	{#if isMenuOpen}
		<div class="md:hidden bg-white dark:bg-slate-900 border-b dark:border-slate-800 p-6 space-y-6">
...
<footer class="bg-slate-900 text-slate-500 py-20 px-4">
	<div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-12">
		<div class="space-y-4 text-center md:text-left">
			<div class="flex items-center justify-center md:justify-start gap-2">
				<div class="w-6 h-6 bg-psan-green rounded shadow-lg shadow-psan-green/20"></div>
				<span class="text-white font-black tracking-tighter text-2xl uppercase">{data.settings?.site_title || 'PSANBLOG'}</span>
			</div>
