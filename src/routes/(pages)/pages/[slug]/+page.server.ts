...
export const load: PageServerLoad = async ({ params, locals }) => {
	...
	if (post.visibility === 'vip' && (!user || !['vip', 'editor', 'admin'].includes(user.role)))
		throw error(403, 'VIP required');
...
export const actions: Actions = {
	addComment: async ({ request, params, locals }) => {
		...
		if (!content || content.length < 1)
			return fail(400, { message: 'コメントを入力してください。' });

		const commentId = generateIdFromEntropySize(10);
		db.prepare(
			'INSERT INTO comment (id, post_id, author_id, parent_id, content, created_at) VALUES (?, ?, ?, ?, ?, ?)'
		).run(commentId, params.slug, locals.user?.id || null, parentId || null, content, Date.now());

		if (parentId) {
			try {
				const parentComment = db
					.prepare('SELECT author_id FROM comment WHERE id = ?')
					.get(parentId) as any;
				if (parentComment?.author_id && parentComment.author_id !== locals.user?.id) {
					const user = db
						.prepare('SELECT notification_enabled FROM user WHERE id = ?')
						.get(parentComment.author_id) as any;
					if (user?.notification_enabled) {
						db.prepare(
							'INSERT INTO notification (id, user_id, type, content, link, created_at) VALUES (?, ?, ?, ?, ?, ?)'
						).run(
							generateIdFromEntropySize(10),
							parentComment.author_id,
							'reply',
							'あなたのコメントにリプライが届きました。',
							`/pages/${params.slug}#comments`,
							Date.now()
						);
					}
				}
