...
{#if data.user?.role === 'admin' || data.user?.role === 'editor'}
	<div class="fixed bottom-8 right-8 z-[100]">
		<a href="/dashboard/pages/home" class="flex items-center gap-2 bg-psan-green text-white px-6 py-3 rounded-full font-black shadow-2xl hover:scale-105 transition-all uppercase tracking-widest text-xs border-4 border-white dark:border-slate-900">
...
<!-- メインコンテンツ -->
<section class="relative pt-20 pb-20 overflow-hidden transition-colors">
	<div class="max-w-7xl mx-auto px-4 relative">
		<div class="max-w-5xl prose prose-xl dark:prose-invert prose-headings:font-black prose-headings:tracking-tighter prose-p:font-medium">
			<!-- 全画面 Editor.js コンテンツ -->
			{@html data.homeHtml}
		</div>
	</div>
...
<!-- 記事リスト (Editor.js の下、または必要に応じて削除も可) -->
